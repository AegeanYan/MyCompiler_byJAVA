...
public class IRBuilder implements ASTVisitor {
    ...
    public void visit(VarDefNode node) {
        if (preCheck){
            ...
            if (curClass != null){
                ...
                if (node.init != null){
                    thisStoreReg = cscope.getVarReg("this" , true);
    ...
    public void visit(ForStmtNode node) {
        ...
        }else condTailBlock.addInstr(new Jump(exeHeadBlock));
    ...
    public void visit(RetStmtNode node) {
        ...
        curBlock.addInstr(new Ret(retVal));
        curBlock.terminator = new Ret(retVal);
    }
    ...
    public void visit(UnaryExprNode node) {
        ...
        if (load instanceof VirtualReg){
            switch (node.op){
                ...
                default: throw new IRError("unknown unary" , node.pos);
            }
        }else {
            switch (node.op){
                case LNOT : {
    ...
    public void visit(BinaryExprNode node) {
        ...
        if (node.op != BinaryExprNode.BinaryOp.ANDAND && node.op != BinaryExprNode.BinaryOp.OROR && node.op != BinaryExprNode.BinaryOp.ASSIGNEQ){
            ...
            if (lhs instanceof VirtualReg || rhs instanceof VirtualReg){
                switch (node.op){
                    ...
                    case LT : case GT : case LEQ : case GEQ:{
                        ...
                        if (node.lhs.getValueType() instanceof IntegerType){
                            switch (node.op){
                                case LT:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.slt , lhs , rhs));break;
                                case GT:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.sgt , lhs , rhs));break;
                                case LEQ:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.sle , lhs , rhs));break;
                                case GEQ:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.sge , lhs , rhs));break;
                            }
                        }else {
                            ArrayList<IRConstant> args = new ArrayList<>();
                            args.add(lhs);
                    ...
                    case SUB: case MUL: case DIV: case MOD:case LSH: case RSH: case AND: case XOR:case OR:{
                        result = new VirtualReg(node.lhs.getValueType() , curFunc.takeLabel());
                        switch (node.op){
                            case SUB:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.sub , lhs , rhs));break;
                            case MUL:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.mul , lhs , rhs));break;
                            case DIV:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.sdiv , lhs , rhs));break;
                            case MOD:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.srem , lhs , rhs));break;
                            case LSH:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.shl , lhs , rhs));break;
                            case RSH:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.ashr , lhs , rhs));break;
                            case AND:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.and , lhs , rhs));break;
                            case XOR:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.xor , lhs , rhs));break;
                            case OR:curBlock.addInstr(new Binary((VirtualReg) result , Binary.BiOp.or , lhs , rhs));break;
                        }
                        break;
                    }
                    case EQ:case NEQ:{
                        ...
                        if (node.lhs.getValueType() instanceof PointerType &&node.rhs.getValueType() instanceof PointerType && node.lhs.getValueType().dePointed() instanceof IntegerType && ((IntegerType) node.lhs.getValueType().dePointed()).width == 8){
                            ...
                            switch (node.op){
                                case EQ:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.eq , lhs , rhs));break;
                                case NEQ:curBlock.addInstr(new Icmp((VirtualReg) result , Icmp.CmpOp.ne , lhs , rhs));break;
                            }
                        }
                        break;
                    }
                ...
                switch (node.op){
                    ...
                    case LT:case GT:case LEQ:case GEQ:{
                        ...
                        switch (node.op){
                            case LT:result = new BoolConstant(l < r);break;
                            case GT:result = new BoolConstant(l > r);break;
                            case LEQ:result = new BoolConstant(l <= r);break;
                            case GEQ:result = new BoolConstant(l >= r);break;
                        }
                        break;
                    }
                    case SUB:case MUL:case DIV:case MOD:case LSH:case RSH: case AND: case XOR:case OR:{
                        ...
                        switch (node.op){
                            case SUB: result = new IntConstant(l - r);break;
                            case MUL: result = new IntConstant(l * r);break;
                            case DIV: result = new IntConstant(l / r);break;
                            case MOD: result = new IntConstant(l % r);break;
                            case LSH: result = new IntConstant(l << r);break;
                            case RSH: result = new IntConstant(l >> r);break;
                            case AND: result = new IntConstant(l & r);break;
                            case XOR: result = new IntConstant(l ^ r);break;
                            case OR:  result = new IntConstant(l | r);break;
                        }
                        break;
                    }
                    case EQ:case NEQ:{
            ...
            if (node.op == BinaryExprNode.BinaryOp.ASSIGNEQ){
                ...
                if (node.op == BinaryExprNode.BinaryOp.ANDAND)rootBlock.addInstr(new Branch(firstReg , alterBlock , resultBlock));
                ...
                if (node.op == BinaryExprNode.BinaryOp.ANDAND)paths.add(new Pair<>(new BoolConstant(false) , rootBlock.label));
    ...
    public void visit(FuncCallExprNode node) {
        ...
        resultReg = callFunc.type instanceof VoidType ? null : new VirtualReg(curFunc.type, callFunc.takeLabel());
    ...
    public void visit(IndexExprNode node) {
        ...
        if (!((node.array.getValueType()) instanceof PointerType))throw new RuntimeException("array must be pointer type");
    ...
    public void visit(MemberAccessExprNode node) {
        ...
        if (!node.forfunc){
            IRType obType = node.object.getValueType();
            assert obType instanceof StructType;
            IRType memType = targetModule.getClassMemberType(((StructType) obType).name , node.member.name);
            VirtualReg ptrReg = new VirtualReg(new PointerType(memType) , curFunc.takeLabel());
            ArrayList<IRConstant> offsets = new ArrayList<>();
            offsets.add(new IntConstant(0));
            offsets.add(new IntConstant(targetModule.getClassMemberIndex(((StructType) obType).name , node.member.name)));
            curBlock.addInstr(new Gep(ptrReg , thisReg , offsets));
            node.addr = ptrReg;
            node.value = null;
        }else {
            ...
            if (node.object.getValueType() instanceof StructType){
                funcArgs.add(0 , thisReg);
                String className = ((StructType) node.object.getValueType()).name;
    ...
    VirtualReg multiAllocate(NewNode node , IRType bottomType , ArrayList<IRConstant> sizes , int depth){
        ...
        if (indexSize instanceof IntConstant)arraySize = new IntConstant(((IntConstant)byteSize).value * ((IntConstant) indexSize).value);
        else {
        ...
        exeHeadBlock = new IRBasicBlock(curFunc , curFunc.takeLabel());
        curBlock = exeHeadBlock;
        elementReg = multiAllocate(node , bottomType , sizes , depth + 1);
        exeTailBlock = curBlock;
    ...
    public void visit(StringValNode node) {
        String constantName = ".str." + targetModule.strConstants.size();
        ...
        targetModule.strConstants.add(new Pair<>(llvmStr.toString() , strConstReg));
    ...
    public void visit(ThisValNode node) {
        VirtualReg thisStoreReg = cscope.getVarReg("this" , true);
...
//    public IRBuilder irBuilderInit(){
//        this.gscope.Class_Table.forEach((classname , classScope) -> {
//            switch (classname){
//                case "int" -> this.typeTable.put("int" , new IntegerType(32));
//                case "bool" -> this.typeTable.put("bool" , new BoolType());
//                case "string" -> this.typeTable.put("string" , new PointerType(new IntegerType(8)));
//                default -> {
//                    StructType newClass = new StructType(classname);
//                    this.typeTable.put(classname , new PointerType(newClass));
//                    this.targetModule.addClassType(newClass);
//                }
//            }
//        });
//        this.typeTable.put("void" , new VoidType());
//        this.gscope.Func_Table.forEach((funcname , funcnode) -> {
//            FuncType funcType = new FuncType(getIrType(funcnode.retnode));
//            if (funcnode.paralist != null) funcnode.paralist.forEach(tmp -> {
//                IRType argType = getIrType(tmp.type);
//                funcType.addParameter(argType,tmp.id);
//            });
//
//            IRFunction tmp_func = new IRFunction("func_"+funcname , funcType);
//            if (funcnode.isBuiltin) tmp_func.setBuiltin();
//            funcTable.put(tmp_func.name , tmp_func);
//            targetModule.addFunc(tmp_func);
//        });
//        this.gscope.Class_Table.forEach((classname , classscope) -> {
//            switch (classname){
//                case "int" , "bool" ->{}
//                default -> {
//                    IRType tmp_type = typeTable.get(classname).dePointed();
//                    assert classscope.Class_Table.size() == 0;
////                    if (!classname.equals("string")) classscope.Var_Table.forEach((ider , tmpnode) -> {
////                        ((StructType) tmp_type).addMember(ider , getIrType(tmpnode));
////                    });//rest
//                    classscope.Var_Table.forEach((ider , tmpnode) -> {
//                        ((StructType) tmp_type).addMember(ider , getIrType(tmpnode));
//                    });
//                    classscope.Func_Table.forEach((funcname , funcnode) -> {
//                        IRType returnType = getIrType(funcnode.retnode);
//                        FuncType funcType = new FuncType(returnType);
//                        IRType argType = new PointerType(tmp_type);
//                        funcType.addParameter(argType , "this_");
//                        if (funcnode.paralist != null){
//                            for (VarDefNode cd : funcnode.paralist){
//                                argType = getIrType(cd.type);
//                                funcType.addParameter(argType , cd.id);
//                            }
//                        }
//                        IRFunction func_ = new IRFunction("class_" + classname + "_"  + funcname , funcType);
//                        if (funcnode.isBuiltin) func_.setBuiltin();
//                        funcTable.put(func_.name , func_);
//                        targetModule.addFunc(func_);
//
//                    });
//
//                    if (!classname.equals("string") && funcTable.get("class_" + classname + "_" + classname) == null){
//                        FuncType funcType = new FuncType(new VoidType());
//                        IRType argType = new PointerType(tmp_type);
//                        funcType.addParameter(argType , "this_");
//                        IRFunction func_ = new IRFunction("class_" + classname + "_" + classname , funcType);
//                        func_.addParameter(new Value("this" , argType));
//                        IRBasicBlock block_ = new IRBasicBlock(func_.name , func_);
//                        new Ret(new Value("Auto_construct" , new VoidType()) , block_);
//                        funcTable.put(func_.name , func_);
//                        targetModule.addFunc(func_);
//                    }
//                }
...
//
//
//    public IRType getIrType(ReturnTypeNode retnode){
//        IRType returnType = typeTable.get(retnode.retType);
//        if (retnode instanceof ArrayTypeNode) returnType = new PointerType(returnType , ((ArrayTypeNode) retnode).dims);
//        return returnType;
//    }
//
//
//    public Value getStringPtr(Value base){
//        Gep ptr = new Gep(new PointerType(new IntegerType(8)) , base , curBlock);
//        ptr.addIndex(new IntConstant(0)).addIndex(new IntConstant(0));
//        return ptr;
//    }
//
//    public void memoryStore(Value value , Value addr){
//        new Store(value , addr , curBlock);
//    }
